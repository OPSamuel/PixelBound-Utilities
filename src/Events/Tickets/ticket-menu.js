...
const followUpProgress = new Map();
    ...
    async execute(interaction) {
        ...
        if (interaction.isStringSelectMenu() && interaction.customId === 'ticket_dropdown') {
            ...
            if (!ticketChannel1) {
                ...
                // Defer the reply to allow for multiple actions
                ...
                await interaction.editReply({ embeds: [followupembed] });

                // Create the ticket channel
                const ticketCategoryId = '1353384018434986064';
                ...
                // Create welcome embed and buttons
                ...
                // Create persistent buttons that won't expire
                ...
                // Additional embed based on ticket type
                const globalFooter = {
                    text: 'PixelBound Entertainment - Ticket System',
                    iconURL: 'https://cdn.discordapp.com/attachments/1188570570288275600/1353296064928813127/cb3ba39c559e8033534fddcc375a658b.png',
                };
                ...
                // User already has a ticket open
                await interaction.deferReply({ ephemeral: true });
                await interaction.editReply({ 
                    content: `You already have a ticket open! Head over to it: <#${ticketChannel1.id}>` 
                });
                ...
                try {
                    ...
                    console.error('Failed to reset dropdown:', error);
                }
            }
        }
        ...
        if (interaction.isStringSelectMenu() && interaction.customId === 'app_role_select') {
            ...
            if (interaction.user.id === ticketCreator.id) {
                ...
                // Defer the reply to allow for message editing
                await interaction.deferReply({ ephemeral: true });
                ...
                try {
                    ...
                    await originalMessage.edit({
                        components: [staffRoleMenuDisabled, staffButtons]
                    });
                    ...
                    // Add the "Apply" button (persistent)
                    const applyButton = new ButtonBuilder()
                        .setCustomId(`apply_${selectedRole}`)
                        .setLabel('Apply')
                        .setStyle(ButtonStyle.Primary);
        ...
        if (interaction.isButton() && interaction.customId.startsWith('apply_')) {
            const selectedRole = interaction.customId.replace('apply_', '');
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId.startsWith('accept_') || interaction.customId.startsWith('deny_')) {
                try {
                    ...
                    const TICKET_CATEGORY_ID = '1353384018434986064';
                    ...
                    // Find and update the application message in the ticket channel
